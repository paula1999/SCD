...
public:
    Estanco ();
    bool obtenerIngrediente (int num_fumador);
...
bool Estanco::obtenerIngrediente (int num_fumador){
    if (num_fumador != mostrador)
        fumador[num_fumador].wait(); // Espera a que esté su ingrediente en el mostrador
    
    if (num_fumador == 2 && contador == 4){
        mtx.lock();
        cout << "Cada 4 veces no fumo\n";
        mtx.unlock();
...
void funcion_hebra_fumador (MRef<Estanco> estanco, int num_fumador){
    ...
    while (true){
        fuma = estanco->obtenerIngrediente (num_fumador);
