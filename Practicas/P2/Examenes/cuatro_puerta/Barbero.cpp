...
mutex mtx;
const int n = 12, MAX_ESPERA = 5;

class Barberia : public HoareMonitor{
private:
    CondVar sala_espera, silla, barbero, puerta;
...
void Barberia::cortarPelo (int num_cliente){
    if (sala_espera.get_nwt() >= MAX_ESPERA){
        mtx.lock();
        cout << "La sala de espera esta LLENA, el cliente " << num_cliente << " espera en la puerta\n";
        mtx.unlock();
