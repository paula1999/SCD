...
void funcion_buffer(){
   ...
   for (unsigned int i = 0; i < num_items*2; i++){
      etiqueta_consumidor_valido = buffer[primera_libre-1] % 2;
      ...
      if (estado.MPI_SOURCE < num_productores){
            ...
            cout << "Buffer va a enviar valor " << valor  << " a " << estado.MPI_SOURCE-num_productores-1<< endl;

            MPI_Ssend(&valor, 1, MPI_INT, estado.MPI_SOURCE, valor%2, MPI_COMM_WORLD);
      }
   }
}
